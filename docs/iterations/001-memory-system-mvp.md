...
- 增加独立的自然语言记忆抽取器，只生成待确认候选，不直接写入长期记忆。
- 根据当前用户问题进行混合排序检索，只向模型注入最相关的 Top-K 记忆。
- 增加记忆版本和有效时间区间，支持偏好变化和历史追踪。

实现说明：

- 抽取器优先识别“我喜欢”“我通常”“以后都这样”等稳定表达，并排除“今天”“这次”“现在”等临时表达。
- 自然语言候选与重复操作候选复用相同的确认、拒绝和权限隔离流程，未经确认不会成为长期记忆。
- 混合检索综合词项相关性、置信度、重要性、更新时间和访问次数进行可解释排序，默认只注入前 6 条，可通过 `retrieval_top_k` 调整。
- 检索命中的记忆会更新访问次数和最后访问时间，后续排序能够反映实际使用频率。
- 记忆新增 `importance`、`access_count`、`last_accessed_at`、`valid_from`、`valid_to` 和 `version` 字段。
- 每次确认或显式修改都会生成新版本；旧版本写入结束有效时间，新版本保持当前有效。
- Repository 启动时自动迁移阶段二至阶段四创建的 SQLite 表，不需要重建已有数据库。
- 本阶段未引入 Embedding 或向量数据库依赖；数据规模达到阶段四评估阈值后，可在现有混合排序前增加向量召回层。
...
15. 稳定自然语言偏好只生成候选，临时表达不会生成候选，且候选未经确认不会进入长期记忆。
16. 模型每轮只接收与当前问题最相关的 Top-K 条长期记忆，检索访问次数会被记录。
17. 用户修改偏好后可以查询完整版本历史，旧版本和当前版本具有连续的有效时间区间。
