...
P1 改造: 工具由 `build_automation_tools(runtime)` 工厂以闭包持有 runtime，
不再读模块级 `_runtime` 单例；runtime=None 时返回空列表（自动化未启用，
Agent 根本看不到这些工具，比旧行为"调用时报 '尚未初始化'"更安全）。
"""
...
def _parse_actions(actions: list) -> list[ScheduledActionInput]:
    return [
...
def _build_automation_tools(runtime) -> list[StructuredTool]:
    def create_scheduled_routine(
        ...
        routine, tasks = runtime.create_scheduled_routine(
            home_id, user_id, name, anchor_at, _parse_actions(actions)
        )
    ...
    def create_vehicle_arrival_routine(
        ...
        home_id, user_id = _identity(config)
        routine = runtime.create_vehicle_arrival_routine(
            home_id, user_id, vehicle_id, name, _parse_actions(actions)
        )
    ...
    def schedule_wake_routine(wake_at_iso: str, config: RunnableConfig = None) -> str:
        ...
        routine, tasks = runtime.schedule_wake(home_id, user_id, wake_at)
    ...
    return [
        StructuredTool.from_function(
            ...
            description=create_scheduled_routine.__doc__,
            args_schema=ScheduledRoutineInput,
            infer_schema=False,
        ),
        StructuredTool.from_function(
            create_vehicle_arrival_routine,
            name="create_vehicle_arrival_routine",
            description=create_vehicle_arrival_routine.__doc__,
            args_schema=VehicleRoutineInput,
            infer_schema=False,
        ),
        StructuredTool.from_function(
            schedule_wake_routine,
            name="schedule_wake_routine",
            description=schedule_wake_routine.__doc__,
        ),
        StructuredTool.from_function(
            enable_vehicle_arrival_routine,
            name="enable_vehicle_arrival_routine",
            description=enable_vehicle_arrival_routine.__doc__,
        ),
        StructuredTool.from_function(
            list_automation_routines,
            name="list_automation_routines",
            description=list_automation_routines.__doc__,
        ),
        StructuredTool.from_function(
            cancel_automation_routine,
            name="cancel_automation_routine",
            description=cancel_automation_routine.__doc__,
        ),
    ]


def build_automation_tools(runtime) -> list[StructuredTool]:
    """构建自动化工具集；runtime=None（自动化未启用）时返回空列表。"""
    if runtime is None:
        return []
    return _build_automation_tools(runtime)


__all__ = ["build_automation_tools"]
